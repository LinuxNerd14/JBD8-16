...
reset: ; on CPU restart
 ldx #$ff
 txs
 cli ; clear interupt disable, aka allows masked interupts

 lda #$82 ; 10000010 enables interupts from CA1
 sta IER
 lda #$00 ; sets PCR to all zero which sets CA1 on the VIA to negative active edge
 sta PCR
...
main:
 ...
 sei ; disables maskable interupts for data sensitive operation
 lda counter
 sta value
 lda counter + 1
 sta value + 1
...
irq:
 ...
 inc counter + 1
exit_irq:
 ldx #$ff ; loop langth 255
delay:
 dex
 bne delay ; loop for anti bounce from button
 bit PORTA
